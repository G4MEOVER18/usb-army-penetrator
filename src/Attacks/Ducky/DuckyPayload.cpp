...
static std::vector<std::string> cmdLineLines;
...
static std::string readLineFromFileOrCmdLine(const std::string &filename, const int lineNumber)
{
    ...
    {
        // we are reading from a cmdline — indexed by line number
        Debug::Log.info(LOG_DUCKY, "Reading cmd line " + std::to_string(lineNumber));

        if (lineNumber >= 0 && lineNumber < (int)cmdLineLines.size())
        {
            return cmdLineLines[lineNumber];
        }
        return ""; // EOF
    }
    else
...
void DuckyPayload::setPayloadCmdLine(const std::string &cmdLine)
{
    ...
    // Split cmdLine by \n into individual lines so the reader can serve
    // them one at a time (indexed by lineNumber from the DuckyInterpreter)
    size_t start = 0;
    while (start <= cmdLine.size())
    {
        size_t end = cmdLine.find('\n', start);
        if (end == std::string::npos) end = cmdLine.size();
        std::string line = cmdLine.substr(start, end - start);
        if (!line.empty() && line.back() == '\r') line.pop_back();
        if (!line.empty()) cmdLineLines.push_back(line);
        if (end >= cmdLine.size()) break;
        start = end + 1;
    }

    if (!cmdLineLines.empty())
    {
        localCmdLineToExecute = "cmdline"; // non-empty marker to trigger execution
        duckyFileParser.Restart();
    }
}
