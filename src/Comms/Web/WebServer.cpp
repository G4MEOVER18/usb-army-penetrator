...
static void webRequestHandler(AsyncWebServerRequest *request)
{
  ...
  else if (url == "/payloadinfo")
  {
    // Stream JSON directly to avoid building large document in heap (OOM with 100+ files)
    ...
    for (const auto &filename : Devices::Storage.listFiles())
    {
      // Only process DuckyScript files
      std::string fname(filename.c_str());
      if (fname.size() < 3 || fname.substr(fname.size() - 3) != ".ds")
        continue;

      if (!first) response->print(",");
      first = false;

      // Determine category from filename prefix (strip leading '/' from path)
      std::string basename = (fname.size() > 1 && fname[0] == '/') ? fname.substr(1) : fname;
      std::string category = "misc";
      if (basename.rfind("wifi_", 0) == 0)        category = "wifi";
      else if (basename.rfind("ble_", 0) == 0)    category = "ble";
      else if (basename.rfind("hid_", 0) == 0)    category = "hid";
      else if (basename.rfind("exfil_", 0) == 0)  category = "exfil";
      else if (basename.rfind("recon_", 0) == 0)  category = "recon";
      else if (basename.rfind("fun_", 0) == 0)    category = "fun";
      else if (basename.rfind("usb_", 0) == 0)    category = "usb";
      bool hid_required = (category == "hid");

      // Read first REM line as description
      std::string desc;
      auto file = Devices::Storage.openFile(fname.c_str(), "r");
      if (file.available())
      {
        char buf[128];
        int idx = 0;
        bool found = false;
        while (file.available() && idx < 127 && !found)
        {
          char c = file.read();
          if (c == '\n')
          {
            buf[idx] = '\0';
            std::string line(buf);
            if (line.size() >= 4 && line.substr(0, 4) == "REM ")
            {
              desc = line.substr(4);
              if (!desc.empty() && desc.back() == '\r') desc.pop_back();
              found = true;
            }
            if (idx > 0 && !found) break; // non-empty non-REM first line, stop
            idx = 0;
          }
          else
          {
      ...
      file.close();
      if (desc.empty()) desc = basename;

      // Write JSON object directly to stream — no heap allocation for full array
      response->print("{\"name\":\"");
      for (char c : fname) {
        if (c == '"') response->print("\\\"");
      ...
      response->print("\",\"category\":\"");
      response->print(category.c_str());
      response->print("\",\"hid_required\":");
      response->print(hid_required ? "true" : "false");
  ...
  else if (url == "/setusbstorage" && preferences != nullptr)
  {
    preferences->putUShort("usbDeviceType", (uint16_t)USBDeviceType::Serial);
    preferences->putUShort("usbClassType",  (uint16_t)USBClassType::Storage);
    request->send(200, "text/plain", "USB mode set to Storage. Rebooting...");
    delay(500);
    ESP.restart();
  }
  else if (url == "/setusbhid" && preferences != nullptr)
  {
    preferences->putUShort("usbDeviceType", (uint16_t)USBDeviceType::Serial);
    preferences->putUShort("usbClassType",  (uint16_t)USBClassType::HID);
    request->send(200, "text/plain", "USB mode set to HID. Rebooting...");
    delay(500);
    ESP.restart();
  }
  else if (url == "/setusbncm" && preferences != nullptr)
  {
    preferences->putUShort("usbDeviceType", (uint16_t)USBDeviceType::NCM);
    preferences->putUShort("usbClassType",  (uint16_t)USBClassType::None);
    request->send(200, "text/plain", "USB mode set to NCM. Rebooting...");
    delay(500);
    ESP.restart();
  }
  else if (url == "/setusbserial" && preferences != nullptr)
  {
    preferences->putUShort("usbDeviceType", (uint16_t)USBDeviceType::Serial);
    preferences->putUShort("usbClassType",  (uint16_t)USBClassType::None);
    request->send(200, "text/plain", "USB mode set to Serial. Rebooting...");
    delay(500);
    ESP.restart();
  }
  else if (url == "/setusbnone" && preferences != nullptr)
  {
    preferences->putUShort("usbDeviceType", (uint16_t)USBDeviceType::None);
    preferences->putUShort("usbClassType",  (uint16_t)USBClassType::None);
    request->send(200, "text/plain", "USB disabled. Rebooting...");
    delay(500);
    ESP.restart();
  }
