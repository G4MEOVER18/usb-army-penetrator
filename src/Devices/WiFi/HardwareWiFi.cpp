...
#define WIFI_AP_NAME_DEFAULT            "RadioRemote"

#define WIFI_AP_PWD                     "wifi-pwd"
#define WIFI_AP_PWD_DEFAULT             "KiCinference$94RC"
...
void HardwareWiFi::begin(Preferences& prefs)
{
  ...
  if (desiredWiFiState == true)
  {
    if (wifiAPMode)
    {
      ...
      bool apOk = WiFi.softAP(wiFiApName, wiFiApPwd);
#ifdef CYD_ESP32
      Serial.printf("[WiFi] softAP=%d mode=%d IP=%s SSID=%s\n", apOk, WiFi.getMode(), WiFi.softAPIP().toString().c_str(), wiFiApName.c_str());
#endif
    }
    else
    {
